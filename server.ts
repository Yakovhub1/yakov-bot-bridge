...
const response = await fetch(listUrl, { headers: { "Authorization": "Bearer " + AIRTABLE_API_KEY } });
...
for (const record of data.records) {
  ...
  if (f.Status === "InProgress" && f.LastInteraction && f.nudge_sent !== true) {
    const lastTime = DateTime.fromISO(f.LastInteraction).setZone("Asia/Jerusalem");
    const diff = now.diff(lastTime, "minutes").minutes;
    if (diff >= 2) {
  ...
  // 2. תזכורת בוקר (10:00 - 10:15)
  if (hour === 10 && minute >= 0 && minute = 0 && minute = now.startOf("day")) { isActive = true; }
    }
...
return new Response("System Live");
}, { port: 8080 });
